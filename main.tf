locals {
  public_subnets_length   = max(length(var.public_subnets), length(var.public_subnet_ipv6_prefixes))
  private_subnets_length  = max(length(var.private_subnets), length(var.private_subnet_ipv6_prefixes))
  database_subnets_length = max(length(var.database_subnets), length(var.database_subnet_ipv6_prefixes))

  max_subnet_length = max(
    local.private_subnets_length,
    local.public_subnets_length,
    local.database_subnets_length,
  )
...
locals {
  create_public_subnets = local.create_vpc && local.public_subnets_length > 0
}

resource "aws_subnet" "public" {
  count = local.create_public_subnets && (!var.one_nat_gateway_per_az || local.public_subnets_length >= length(var.azs)) ? local.public_subnets_length : 0
...
resource "aws_route_table_association" "public" {
  count = local.create_public_subnets ? local.public_subnets_length : 0
...
locals {
  create_private_subnets = local.create_vpc && local.private_subnets_length > 0
}

resource "aws_subnet" "private" {
  count = local.create_private_subnets ? local.private_subnets_length : 0
...
locals {
  create_database_subnets     = local.create_vpc && local.database_subnets_length > 0
  create_database_route_table = local.create_database_subnets && var.create_database_subnet_route_table
}

resource "aws_subnet" "database" {
  count = local.create_database_subnets ? local.database_subnets_length : 0
...
resource "aws_route_table" "database" {
  count = local.create_database_route_table ? var.single_nat_gateway || var.create_database_internet_gateway_route ? 1 : local.database_subnets_length : 0
...
resource "aws_route" "database_nat_gateway" {
  count = local.create_database_route_table && !var.create_database_internet_gateway_route && var.create_database_nat_gateway_route && var.enable_nat_gateway ? var.single_nat_gateway ? 1 : local.database_subnets_length : 0
...
resource "aws_route" "database_dns64_nat_gateway" {
  count = local.create_database_route_table && !var.create_database_internet_gateway_route && var.create_database_nat_gateway_route && var.enable_nat_gateway && var.enable_ipv6 && var.private_subnet_enable_dns64 ? var.single_nat_gateway ? 1 : local.database_subnets_length : 0
...
resource "aws_route" "private_ipv6_egress" {
  count = local.create_vpc && var.create_egress_only_igw && var.enable_ipv6 ? local.private_subnets_length : 0
